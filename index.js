const sessionName = 'putra'
const donet = 'https://saweria.co/hextra'
const owner = ['62895806424898']
const { default: hextraConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, getContentType } = require("@adiwajshing/baileys")
...
async function startHisoka() {
    ...
    const client = hextraConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Wa-OpenAI - hextra','Safari','3.0'],
        auth: state
    })
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./hextra")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    client.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Delete Session file hextra.json and Scan Again.`); process.exit(); }
